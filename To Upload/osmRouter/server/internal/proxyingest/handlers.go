...
// VerifyDevice validates an api_key (presented by a tunnel client during
// `hello`) against the devices table. Returns the device + user identity
// on success so the proxy can scope the tunnel.
