...
func (s *Service) Create(ctx context.Context, userID string, in CreateInput, ip, ua string) (*models.Subdomain, error) {
	...
	// UPSERT by (parent_domain_id, prefix). Without this, every "Create"
	// for the same apex/subdomain inserts a fresh row, and the Domains
	// page in the Mac app accumulates duplicates (1 visible bug per repeat
	// click). Each (domain, prefix) pair must be at most one Subdomain row.
	var sd models.Subdomain
	err := s.db.WithContext(ctx).
		Where("parent_domain_id = ? AND prefix = ?", domain.ID, prefix).
		First(&sd).Error
	switch {
	case err == nil:
		// Existing row — update target_port if it changed. Don't touch
		// binding fields (BoundDeviceID/BoundAt); those live on the
		// /bind + /unbind endpoints.
		if sd.TargetPort != in.TargetPort {
			if err := s.db.WithContext(ctx).Model(&sd).
				Update("target_port", in.TargetPort).Error; err != nil {
				return nil, err
			}
			sd.TargetPort = in.TargetPort
		}
