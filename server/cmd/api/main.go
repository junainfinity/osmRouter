...
func main() {
	...
	// Seed the subscription tiers — idempotent: brand-new rows get created,
	// legacy rows have their empty/zero fields back-filled, admin overrides
	// (any non-empty value) are left alone.
	if err := db.SeedPlans(database); err != nil {
		logger.Error("seed plans failed", "err", err)
		os.Exit(1)
	}
